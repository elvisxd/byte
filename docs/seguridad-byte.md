...
- [x] **Un solo worker**: los runs y los nonces ya canjeados viven en memoria del proceso, así que con dos el SSE no encuentra runs del otro y el "un solo uso" de `resume_token` y `events_token` vale una vez por worker. Se chequea `WEB_CONCURRENCY` al arrancar y **se corta salvo que sea dev en una máquina de desarrollo**: alcanza con que exista una variable `RAILWAY_*` para que no arranque. Cortar solo con `BYTE_ENV=prod` protegía el caso en que uno se acordó de configurarla y se callaba en el que se la olvidó, que es al revés de lo que hace falta
